...
Get-ADOrganizationalUnit -Identity "OU=$Graduating_Elementary_OU,OU=MiddleSchool,OU=StudentAccounts,DC=vigschools,DC=org" | Set-ADObject -ProtectedFromAccidentalDeletion $true
...
Try {
  ...
  Get-ADOrganizationalUnit -Identity "OU=$Graduating_High_OU,OU=Graduated_Classes,OU=StudentAccounts,DC=vigschools,DC=org" | Set-ADObject -ProtectedFromAccidentalDeletion $true
}
Catch {
